...
lfsr = eeprom_read_byte((uint8_t*)(0x3FC));
bit = (( lfsr >> 0) ^ (lfsr >> 2) ^ (lfsr >> 3) ^ (lfsr >> 4)) & 1;
lfsr = (lfsr >> 1) | (bit << 7);
eeprom_write_byte((uint8_t*)(0x3FC),lfsr);
return lfsr;}
...
{lfsr = (eeprom_read_byte((uint8_t*)0x3FC) << 8) + 
eeprom_read_byte((uint8_t*)0x3FB);}										//Data saved to EEPROM survives resets including POR (power on reset)
...
{eeprom_write_byte((uint8_t*)0x3FC,(lfsr>>8));							//Save every 16th PRN number to EEPROM
eeprom_write_byte((uint8_t*)0x3FB, lfsr);									//This prevents the display from endlessly repeating
