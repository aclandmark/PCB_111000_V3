...
case (signature_bit_3): if((n > 7) && (PGD_resp_H)) Echo = Echo | (1<< (15-n)); break;
case (read_fuse_bits):
case (read_extended_fuse_bits):
case (read_fuse_bits_H): if((n > 7) && (PGD_resp_H)) Echo = Echo | (1<< (15-n));break;
case (read_lock_bits): if((n > 9) && (PGD_resp_H)) Echo = Echo | (1<< (15-n));break;
}}PGC_L;	PGClock_L;  
{ten_msec_delay;}	
return Echo;}                
